...
# still lists every roster entry. Returns one client name per line on stdout.
...
# Resolve a credential env reference. Expands the value, then checks if the
# expanded string is empty. Prints a warning message to stderr when the value
# is empty. Returns 0 if resolved to a non-empty value, 1 if empty.
resolve_credential() {
  local label="$1"
  local raw_val="$2"
  local client_name="$3"
...
flush_and_register() {
  ...
  # Expand env var references for gateway_url.
  local gw_url
  ...
  # Resolve credentials with warnings.
  local gw_token=""
  local cf_id=""
  local cf_secret=""
...
# Walk the roster file line by line.
while IFS= read -r line; do
  ...
  if [[ "$line" =~ ^\[([^]]+)\]$ ]]; then
    # Flush the previous client before starting a new one.
    flush_and_register
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  echo "  Clients in roster:  (see above)"
