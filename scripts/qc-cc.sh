...
blue ""
blue "── 15. B-U6/B-U7/B-U8 — declared-vs-used comparator + ingest parity + offline fixture funnel (Skill 6 unification) ──"
#
...
# 15.1: persona-mismatch.ts exports the B-U6 comparator's full public surface.
check "15.1" "persona-mismatch.ts exports recordPersonaUsedAndCompare + getOpenPersonaMismatch + PERSONA_MISMATCH_EVENT_TYPE" \
...
# 15.2: the comparator dedupes on the (task, declared, used) triple — never
#       re-fires the SAME divergence, never fabricates a mismatch on a NULL
#       declared/used side.
check "15.2" "recordPersonaUsedAndCompare dedupes on (task, declared, used) and fail-softs on a missing declared/used voice" \
...
# 15.3: tasks.ts exports the B-U7 producer-pin helper + the voice-gates-the-group contract.
check "15.3" "tasks.ts exports pinProducerPersonaBundle; voice_persona_id gates the whole producer-pin group" \
...
# 15.4: the skip-branch actually SKIPS — no resolvePersonaAndPin/resolvePersonaPlanAndPin
#       call sits inside the producerVoicePersonaId truthy branch.
check "15.4" "the producer-pin branch never calls resolvePersonaAndPin/resolvePersonaPlanAndPin (real skip, not a re-match in disguise)" \
...
# 15.5: each unit's own dedicated test file is present (so test:unit runs it).
check "15.5" "B-U6/B-U7/B-U8 dedicated test files present" \
...
# 15.6: the B-U8/U22 offline fixture-funnel payload files exist (the fixtures
#       the funnel test above is fed from — a deleted fixture would silently
#       degrade that test to trivially vacuous, not fail loudly).
check "15.6" "B-U8/U22 fixture-funnel payload files present under tests/fixtures/persona-bundles/" \
