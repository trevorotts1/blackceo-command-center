...
#
# Usage:
#   scripts/fix-cron-delivery.sh --dry-run       # print edits, apply none
#   scripts/fix-cron-delivery.sh --apply         # apply the delivery fix
#
# Trevor's Telegram chat_id is read from $TELEGRAM_CHAT_ID, then from
# $OPENCLAW_CONFIG (~/.openclaw/openclaw.json) — it is NEVER hardcoded in this
# script (fleet-wide repo).
#
# Cron jobs identified by name pattern, not hardcoded UUID, so the script
# survives cron re-creation.  The discovery step runs the read-only
# `openclaw cron list` once; every operation after that is an idempotent edit.
#
# Dependencies: openclaw CLI (for cron list / cron edit); jq (JSON parsing).
...
resolve_chat_id() {
  ...
  # 2. openclaw config (telegram allowFrom first entry)
...
if CHAT_ID="$(resolve_chat_id)"; then
  ...
  Set TELEGRAM_CHAT_ID in your environment or ensure
  ~/.openclaw/openclaw.json has channels.telegram.allowFrom[0]."
fi

# ── discover cron jobs ───────────────────────────────────────────────────────

discover_cron_jobs() {
  # Return a JSON array of {name, id} for jobs matching fleet-heartbeat or
  # mission-control-standup (case-insensitive name match).
  local list_json
...
cron_delivery_is_fixed() {
  # $1 = cron job id.  Returns 0 if delivery already has channel=telegram and a
  # non-empty "to" field.  Returns 1 otherwise.
  local id="$1"
  local detail
...
fix_one_cron() {
  ...
  if cron_delivery_is_fixed "$id"; then
    info "  $name ($id): already has explicit delivery (channel=telegram, to set) — skipping (idempotent)"
    return 0
  fi
