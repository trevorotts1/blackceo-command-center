/**
 * Seed the 17 starter SOPs that anchor the Hybrid SOP system / Triad Rule.
 *
 * One SOP per default department slug Skill 23 produces:
 *   ceo, marketing, sales, billing, support, legal, webdev, appdev, graphics,
 *   video, audio, research, comms, crm, openclaw, social, paid-ads
 *
 * Trevor's spec: starters should be a credible baseline, not lorem ipsum.
 * Clients will swap them with their own playbooks over time, so each one is
 * an actual playbook with checklists + success criteria + persona hints.
 *
...
function seed() {
  ...
  for (const sop of SOPS) {
    const existing = queryOne<{ id: string }>('SELECT id FROM sops WHERE slug = ?', [sop.slug]);
    if (existing) {
      console.log(`  [skip] ${sop.slug} (already exists)`);
      skipped++;
      continue;
    }
    const id = uuidv4();
    ...
    console.log(`  [seed] ${sop.slug} (${sop.department})`);
    inserted++;
  }

  console.log(`\n[seed-starter-sops] Done. Inserted ${inserted}, skipped ${skipped} (of ${SOPS.length} total).`);
}
