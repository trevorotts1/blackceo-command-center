...
### Tests
- `tests/unit/persona-artifact-snapshot-scope.test.ts` (new, 10 cases, hermetic — a temp database, real reservations and dispatch sends, no network): the live 13-registered / 6-in-report shape PASSES; an earlier attempt's deliverable is out of scope and out of the verdict; an artifact the current attempt registered and did not snapshot is still a hard gap, and the row that failed it is proved to carry the current execution's id; bytes changed after the report are still a hard gap; a card carrying seven unattributed rows clears the gate on its next attempt with nothing else done to it; and a card with nothing attributed is still measured whole, where an unreported row still fails.
- Also covered there: a fresh card whose producer reported the filename where the UUID belongs PASSES; a report carrying UUIDs matching no current row passes on the bytes; a report whose sha does not match the bytes still fails, and evidence matching nothing is still the missing-snapshot gap; and the rendered instruction names the 201 registration id.
...
- Mutation-proved by RUNNING each revert, not by assertion: dropping the deliverable scoping turns three cases red; dropping the sha256 lane turns the stale-UUID case red; dropping the name lane changes the control's failure reason; dropping the loop-window boundary turns the repaired-history case red while the real-loop case stays green. All restored. `tsc`, `npm run lint` (0 errors) and `npm run build` are clean, and the unit suite is 3,137 passing with zero failures.
