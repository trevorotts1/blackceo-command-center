...
- **The Command Center has one Node runtime identity, and an ABI mismatch is now a named refusal instead of a crash loop.** Nothing named a node version, so `ecosystem.config.cjs` ran `script: 'bash'` with no interpreter and no PATH, `scripts/cc-start.sh` exec'd a bare `node` off whatever pm2 inherited, there was no `.nvmrc`, and `postinstall` compiled better-sqlite3 against whichever node ran npm. Boxes ran the app under node@24 (module ABI 137) while cron and update shells resolved Node 26 (ABI 147), so every update reintroduced a NODE_MODULE_VERSION crash loop. `scripts/lib/node-runtime.sh` is now the single resolver every layer asks. `cc-start.sh` execs the resolved binary and, before exec, compares the artifact's recorded module ABI to its own; a mismatch writes a `native-abi-mismatch` refusal receipt and exits 78, which pm2's `stop_exit_codes` stops and the watchdog's existing refusal-receipt class repairs with one locked atomic deploy.
- **The deploy's own native gate was testing the wrong node.** `_ccbi_native_gate` already accepted a `CCBI_NODE_BIN`, but nothing ever set it, so it fell through to `command -v node`: the deploy shell's node, not the one pm2 execs. A gate that loads the module under a different ABI than the runtime proves nothing about the runtime, which is how this defect survived every deploy meant to catch it. The gate is now fed the resolved runtime, the deploy PATH leads with it so the staged `npm ci`, the build and every rebuild agree, and a failing pre-flight gate is repaired by one automatic rebuild and re-gated rather than aborting with a manual command the deploy could have run itself. A second failure still aborts.
...
### Changed
- **`engines.node` narrows from `^20.19.0 || ^22.13.0 || >=24` to `>=24 <25`, and `update.sh` requires major 24 exactly.** This is deliberate and it narrows what the fleet may run. A range wide enough to admit both halves of an ABI mismatch cannot detect the mismatch: `npm ci` and postinstall compiled the native module for the update shell's node while pm2 loaded it under its own, and both nodes satisfied the old range. Boxes on Node 20, 22 or 26 must move to Node 24 before updating. The preflight says so by name and changes nothing on the box before it fails.
...
- `tests/unit/issue09-node-runtime-identity.test.sh` (49 assertions), also control-first. Keeping the resolver and reverting only its consumers fails 28 of the 49. `tests/unit/update-locked-dependencies.test.ts` now proves v20.19.0, v22.13.0 and v25.0.0 are rejected, the three that used to pass this gate.
