...
- **Two false-red vectors are closed before that restart can fire.** For the first (widest watched threshold + 1) minutes of process uptime a would-be failure reports INDETERMINATE, and `scripts/atomic-deploy.sh` retries exit 3 without ever rolling back, so a deploy health probe cannot roll a box back over a job that has not had time to run. A healthy result is never downgraded, which would otherwise yellow every box for the whole window after every restart. And because `getWatchedJobLiveness()` swallows a read error and reports the job as never observed, one trivial read now proves `job_liveness` is readable before silence is called a stall: a locked database, or a box whose migrations have not created the table, can no longer be restarted for a fault it does not have.
