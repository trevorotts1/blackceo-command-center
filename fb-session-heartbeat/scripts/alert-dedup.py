...
def _msg_first(client, service, message, n) -> str:
    return (
        "[FB-SESSION ALERT] client=%s service=%s\n%s\n%d occurrence(s)."
...
def _msg_still_down(client, service, message, n, oldest_days) -> str:
    return (
        "[FB-SESSION ALERT: STILL DOWN] client=%s service=%s\n%s\n"
        "still down, now %d occurrence(s) recorded, oldest %d day(s)."
...
def _msg_recovery(client, service, message, n) -> str:
    return (
        "[FB-SESSION RECOVERY] client=%s service=%s\n%s\n"
        "%d occurrence(s) cleared." % (client, service, message, n)
    )


def _msg_decision(client, service, episode, message) -> str:
    return (
        "[FB-SESSION DECISION] client=%s service=%s episode=%s\n%s"
...
def _msg_digest(client, lines) -> str:
    ...
    return (
        "[FB-SESSION DIGEST] client=%s (%d deferred/batched item(s) today):\n%s"
