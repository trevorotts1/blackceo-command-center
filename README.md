...
**Current release: v7.1.6 (2026-09-08)** — company-bound Skill 35 publish queue and strict model selection. Publish queue reads, enqueues and mutations resolve tenant and company context server-side and reject cross-company identifiers (F01); provider-verified full model slugs are accepted with no change to selection order or fallback (F31); a strict vision requirement blocks dispatch instead of degrading to a text model, served only by a policy-approved capable vision fallback (F37). Migration 135 is additive and idempotent. The v7.1.5 interview resume work remains included: 24-hour private entry links, bounded 30-day company-bound browser sessions, operator-triggered renewal with delivery safeguards, and legacy sign-in grants requiring a fresh link after upgrade. The portable installer companion remains tracked in [onboarding PR #1043](https://github.com/trevorotts1/openclaw-onboarding/pull/1043). See [interview launch](docs/interview-launch-readiness.md).
...
The highlights below describe earlier releases. The current release is v7.1.6; the complete history is in [CHANGELOG.md](CHANGELOG.md).
