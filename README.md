...
**Current release: v7.1.5 (2026-09-07)** — resume interviews through 24-hour private entry links and bounded 30-day company-bound browser sessions. Submitted answers stay saved across sign-in renewal; unfinished drafts can recover in the same browser, and the interview returns to the next unfinished step. Operator-triggered renewal sends a fresh private link with delivery safeguards. Legacy sign-in grants require a fresh link after upgrade. The portable installer companion remains tracked in [onboarding PR #1043](https://github.com/trevorotts1/openclaw-onboarding/pull/1043). See [interview launch](docs/interview-launch-readiness.md).
...
The highlights below describe earlier releases. The current release is v7.1.5; the complete history is in [CHANGELOG.md](CHANGELOG.md).
