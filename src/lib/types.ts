...
export interface Task {
  ...
  // Persona-blend + audience-confirm mirror columns (migration 089). Written by the
  ...
  // absent on a pre-089 row and on a non-content (no-bundle) task, so no regression.
...
/** A row of the `task_persona_bundle` table (migration 089). */
