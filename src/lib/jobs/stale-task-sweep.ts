...
interface StaleTaskRow {
  ...
  /** FIX-17: owner-kill timestamp (migration 114). Absent on pre-114 DBs. */
...
export async function runStaleTaskSweep(): Promise<StaleSweepResult> {
  ...
  // FIX-17: `killed_at` (migration 114) may be absent on a box mid-roll. The
  // owner-kill guard reads it defensively (see owner-killed.ts), but the SELECT
  // below must not reference a column that does not exist yet — a pre-114 box
  ...
  // and only include it when present. A pre-114 box keeps working with the
