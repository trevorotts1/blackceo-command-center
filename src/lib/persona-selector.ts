 ...
 * Fail-soft: a pre-089 DB (columns/table absent) logs a warning and no-ops rather
...
export function persistPersonaBundle(
  ...
  try {
    ...
    console.warn(`[persona-bundle] persist row failed for task ${taskId} (pre-089 DB?):`, (err as Error).message);
  }
  ...
  try {
    ...
    console.warn(`[persona-bundle] mirror-column update failed for task ${taskId} (pre-089 DB?):`, (err as Error).message);
  }
