 ...
 *
 * When no exact match for the provider's documented default model is found,
 * the fallthrough substitution is RESTRICTED to models the registry records as
 * web-search-capable. A chat/embeddings-only model must never be promoted to
 * "research" just because it is the first (or only) active row for this provider.
 */
export function resolveResearchModel(slug: ResearchProviderSlug, fallback: string): string {
  try {
    ...
    // No exact match for the default -- restrict the substitution to models
    // the registry records as web-search-capable.  A chat/embeddings-only
    // model must never be promoted to "research" just because it is the
    // first (or only) active row for this provider.
