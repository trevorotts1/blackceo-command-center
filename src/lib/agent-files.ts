...
export function checkSharedFileSymlink(name:string,column:string):boolean{if(!SHARED_COLUMNS.has(column))return false;const d=agentDir(name);const fn=FILE_MAP[column];if(!fn)return false;return isSymlink(path.join(d,fn))}
...
export function createAgentFolder(n:string,rl:string,md:string):void{const d=agentDir(n);const md2=path.join(d,'memory');fs.mkdirSync(md2,{recursive:true});let t='Execution';if(md.includes('opus'))t='Strategic';else if(md.includes('perplexity'))t='Research';const tps:Record<string,string>={'SOUL.md':`# ${n}\n\n## Identity\n- **Role:** ${rl}\n- **Model:** ${md}\n- **Tier:** ${t}\n\n## Personality\nDefine this personality, communication style, and values here.\n\n## Boundaries\nWhat this agent should and should not do.\n`,'AGENTS.md':`# ${n} - Workspace Rules\n\n## Role\n${rl}\n\n## Rules\n- Follow instructions precisely\n- Report completion with evidence\n- Escalate blockers to Master Orchestrator\n`,'TOOLS.md':`# ${n} - Tools & Capabilities\n\n## Available Tools\nList the tools, APIs, and integrations this agent has access to.\n\n## Credentials\nReference any API keys or credentials this agent needs (do not store secrets here).\n`,'MEMORY.md':`# ${n} - Long-Term Memory\n\n## Lessons Learned\nCurated memories, decisions, and lessons learned over time.\n\n## Key Decisions\nImportant decisions that should persist across sessions.\n`};for(const[fn,ct]of Object.entries(tps)){const fp=path.join(d,fn);if(isSymlink(fp))continue;if(!fs.existsSync(fp))fs.writeFileSync(fp,ct,'utf-8')}}
export function deleteAgentFolder(n:string):boolean{const d=agentDir(n);if(fs.existsSync(d)){fs.rmSync(d,{recursive:true});return true}return false}
