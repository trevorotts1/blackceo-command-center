...
export function readAgentFiles(name: string): Record<string, string | null> {
  ...
  for (const [c, f] of Object.entries(FILE_MAP)) {
    const fp = path.join(dir, f);
    try { if (fs.existsSync(fp)) r[c] = fs.readFileSync(fp, 'utf-8'); else r[c] = null; } catch { r[c] = null; }
  }
  return r;
}
...
export function writeAgentFile(name: string, column: string, content: string): void {
  const dir = agentDir(name);
  const fn = FILE_MAP[column];
  if (!fn) return;
  const tp = path.join(dir, fn);
  if (SHARED_FILE_COLUMNS.has(column) && isSymlink(tp)) throw new SharedFileSymlinkError(name, column, fn);
...
export function listAgentDailyLogs(name: string): string[] {
  const d = path.join(agentDir(name), 'memory');
  try { if (fs.existsSync(d)) return fs.readdirSync(d).filter(f => f.endsWith('.md')).map(f => f.replace('.md', '')).sort().reverse(); } catch {}
  return [];
}

export function createAgentFolder(name: string, role: string, model: string): void {
  const dir = agentDir(name);
  const md = path.join(dir, 'memory');
  ...
  for (const [fn, c] of Object.entries(t)) {
    const fp = path.join(dir, fn);
    if (isSymlink(fp)) continue;
    if (!fs.existsSync(fp)) fs.writeFileSync(fp, c, 'utf-8');
  }
}

export function deleteAgentFolder(name: string): boolean {
  const d = agentDir(name);
  if (fs.existsSync(d)) { fs.rmSync(d, { recursive: true }); return true; }
  return false;
}
