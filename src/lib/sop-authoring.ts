...
export async function authorSOPForTask(input: AuthorSOPInput): Promise<AuthorResult> {
  try {
    ...
    // through transition() would throw ILLEGAL_TRANSITION. We instead go through
    // transitionWithDeclaredException (U071), the one audited entry point for
    // edges the state machine does not model, which runs the full precondition
    // set (including the presentations certificate gate) before writing.
    if (subTaskId) {
      try {
        ...
        const res = transitionWithDeclaredException({
          taskId: subTaskId,
          to: 'done',
          ...
          extraColumns: { completed_at: fileNow, updated_at: fileNow },
        });
        // transitionWithDeclaredException already wrote both audit rows inside a
        // single transaction. The owner DONE report runs post-commit.
        try {
