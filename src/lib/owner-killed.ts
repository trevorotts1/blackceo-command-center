 ...
 *   1. STRUCTURED — the `tasks.killed_at` column (migration 114). The
 ...
 * pre-migration-114 DB the column is absent (`undefined`), the structured path is
...
export function isOwnerKilled(input: {
  ...
  // 1. Structured column (authoritative — migration 114).
...
/**
 * Defensive read of the `killed_at` column. On a pre-migration-114 DB the column
