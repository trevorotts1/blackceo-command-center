...
/** True when the `workspaces.archived_at` column exists (migration 094 applied). */
...
export function syncDeclinedWorkspaceArchive(
  ...
  if (!hasWorkspaceArchiveColumn(db)) {
    // Pre-094 DB. Report honestly rather than pretending the archive happened.
    console.warn(
      '[C6] workspaces.archived_at absent (migration 094 not applied) — skipping decline archive.',
    );
    return result;
  }
