...
import { notifySocialOwnerPrivate } from '@/lib/notify';
beforeEach(()=>{
...
it('accepts the installed gateway receipt without an optional chatId and pins the agent',async()=>{
 expect(await notifySocialOwnerPrivate(input)).toEqual({status:'accepted',messageId:'42'});
 expect(gateway.call.mock.calls[0][0]).toBe('send');expect(gateway.call.mock.calls[0][1].agentId).toBe('main');
 const first=gateway.call.mock.calls[0][1].idempotencyKey;await notifySocialOwnerPrivate(input);expect(gateway.call.mock.calls[1][1].idempotencyKey).toBe(first);
});
it('rejects a receipt for a different delivery or recipient',async()=>{
 ...
 expect((await notifySocialOwnerPrivate(input)).status).toBe('uncertain');
...
it('does not dispatch if connection or ownership verification fails',async()=>{
 gateway.connect.mockRejectedValue(new Error('offline'));expect((await notifySocialOwnerPrivate(input)).status).toBe('not-dispatched');expect(gateway.call).not.toHaveBeenCalled();
 expect((await notifySocialOwnerPrivate({...input,companyId:'b'})).status).toBe('not-dispatched');
});
