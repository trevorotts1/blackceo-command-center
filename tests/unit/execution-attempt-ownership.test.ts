...
test('one worker capacity and duplicate names have independent stable attempt sessions',()=>{const db=fixture();try{assert.ok(claim(db,'t1','e1').execution);assert.equal(claim(db,'t2','e2').reason,'execution_or_worker_busy');assert.ok(claim(db,'t3','e3').execution);assert.notEqual(latestExecution('t1',db)!.session_id,latestExecution('t3',db)!.session_id);assert.equal(db.prepare('SELECT COUNT(*) n FROM openclaw_sessions').get() && (db.prepare('SELECT COUNT(*) n FROM openclaw_sessions').get() as {n:number}).n,2);}finally{db.close();}});
