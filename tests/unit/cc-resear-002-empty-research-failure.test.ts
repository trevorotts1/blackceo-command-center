 ...
 *
 * THE DEFECT (U087)
 * When runResearch() returns HTTP 200 with an empty `answer`, the route
 * persisted it as a completed search. The rendered markdown showed
 * "(no answer returned)" but the record counted as real.
 *
...
const CLEAR = Object.fromEntries(
  ['PERPLEXITY_FIXTURE_JSON_PATH','OPENAI_FIXTURE_JSON_PATH','OLLAMA_FIXTURE_JSON_PATH','XAI_FIXTURE_JSON_PATH','X_AI_FIXTURE_JSON_PATH'].map(n=>[n,undefined]),
);
...
test('A1: empty answer -> 502', async () => {
  ...
  try {
    ...
    const res = await withEnvAsync({ ...CLEAR, NODE_ENV: 'test', PERPLEXITY_API_KEY: 'k' }, () => POST(req));
    assert.equal(res.status, 502);
    const b = await res.json() as any;
    assert.equal(b.error, 'provider_failed');
    assert.match(b.detail, /empty answer/);
  } finally { globalThis.fetch = orig; }
});

test('A2: whitespace-only answer -> 502', async () => {
  const orig = globalThis.fetch;
  try {
    fakeFetch('  \n\t  ');
    const { POST } = await import('../../src/app/api/operator/research/search/route');
...
test('B1: non-empty 0-cite -> persisted with ungrounded:true', async () => {
  const orig = globalThis.fetch;
  try {
...
test('C1: non-empty with cites -> persisted normally', async () => {
  const orig = globalThis.fetch;
  try {
