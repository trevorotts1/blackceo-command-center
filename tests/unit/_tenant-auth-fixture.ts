...
export function fixtureTenantCookie(host:string):string {
  ...
  if(!secret)return '';
  const payload=Buffer.from(JSON.stringify({purpose:'session',tenantId:'fixture-self',companyId:'default',subject:'owner:fixture',host,installationId:'fixture-install',exp:Date.now()/1000+3600,nonce:'fixture-browser'})).toString('base64url');
