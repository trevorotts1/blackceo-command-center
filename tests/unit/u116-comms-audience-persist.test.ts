 ...
 *
 * Proves:
 *   1. Migration 106 adds `tasks.comms_audience_source` / `tasks.comms_type`,
...
// ── migration 106 additive-only ─────────────────────────────────────────────

test('migration 106 adds comms_audience_source + comms_type columns to tasks, additive-only', () => {
  const cols = queryAll<{ name: string }>('PRAGMA table_info(tasks)').map((c) => c.name);
  assert.ok(cols.includes('comms_audience_source'), 'tasks.comms_audience_source must exist post-106');
  assert.ok(cols.includes('comms_type'), 'tasks.comms_type must exist post-106');
