 ...
 * persona-bundle SUPERSET (migration 089 + the audience-confirm write gate).
 *
 * Proves the CC-owned guarantees end to end WITHOUT Python:
 *
 *   A. Migration 089 — the mirror columns exist on `tasks` and the
...
test.before(async () => {
  ...
  getDb(); // run schema + full migration chain (incl. 089)
...
// ── A. Migration 089 shape ───────────────────────────────────────────────────

test('[089] tasks mirror columns + task_persona_bundle table exist', () => {
  ...
  for (const col of ['voice_persona_id', 'topic_persona_id', 'audience_id', 'audience_label', 'audience_source', 'voice_collapsed', 'blend_directive']) {
    assert.ok(cols.includes(col), `tasks.${col} must exist (migration 089)`);
  }
