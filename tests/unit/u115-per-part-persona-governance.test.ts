 ...
 * (never a new bundle store — master spec L2465), extended by migration 106
...
getDb(); // apply the full migration chain, including 106.
...
// ── migration 106 — additive-only column proof ─────────────────────────────

test('migration 106 adds all 5 per-part columns to task_persona_bundle_scope', () => {
...
test('migration 106 never touches task_persona_bundle (090) — schema byte-identical', () => {
  ...
  const expected =
    ...
    `)`;
  assert.equal(sql!.sql, expected, '090 table schema must be byte-identical pre/post migration 106');
});

test('NO-WEAKENING: migration 106 never touches task_persona_bundle_scope (105)\'s composite UNIQUE(task_id, scope)', () => {
  const taskId = seedTask('constraint probe, post-106');
  run(
  ...
  }, /UNIQUE constraint failed/, 'the composite UNIQUE(task_id, scope) constraint still has teeth after 106');
});
