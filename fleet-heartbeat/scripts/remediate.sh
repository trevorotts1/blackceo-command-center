...
# FIX-RESCUE-10: compact append-only incident index. pattern_flag reads THIS
# (one ~40-byte line per incident, filtered to the recent window) instead of
# perl-scanning the multi-MB prose change-log twice on every fix. change-log.md
# itself is rotated monthly (change-log-YYYY-MM.md) to bound disk growth.
INCIDENTS_TSV="${ROOT}/incidents.tsv"
CHANGE_LOG_MONTH_MARKER="${ROOT}/.change-log-month"
...
mac_ssh_run() {
  local t="$1"; shift
  local remote="$1"
  local stem cid csec cfd proxy ct token_pair
  # Resolve the CF Access service-token stem for THIS client. Without this the
  # stem is empty, read_cf_access_token returns empty id/secret, the tunnel
  # ProxyCommand carries no Access token, and every Mac reachability/SSH check
  # fails (over-escalating to a human instead of self-healing).
  stem=$(_token_stem_for_client "$CLIENT")
...
# FIX-RESCUE-10: one-time backfill of the compact incident index from the
# existing prose change-log, so pattern detection keeps working immediately on
# the first fix after this change ships (no cold-start blind window). Runs only
# when incidents.tsv is absent; parses the "## <ts>, <client>" + "- Failure:
# <class>" pairs the old prose format already carried. Never writes secrets.
backfill_incidents_tsv_if_missing() {
  [ -f "$INCIDENTS_TSV" ] && return 0
  if [ ! -f "$CHANGE_LOG" ]; then
    : > "$INCIDENTS_TSV" 2>/dev/null || true
    return 0
  fi
  perl -e '
    ...
    while (my $line = <$fh>) {
      if ($line =~ /^## (\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}) [^,]*, (.+?)\s*$/) {
        my ($yy,$mm,$dd,$hh,$mi,$c) = ($1,$2,$3,$4,$5,$6);
        $cli = $c;
        eval { $epoch = timelocal(0, $mi, $hh, $dd, $mm-1, $yy-1900); 1 } or $epoch = 0;
      } elsif ($line =~ /^- Failure:\s*(.+?)\s*$/) {
        print "$epoch\t$cli\t$1\tbackfill\n" if $epoch > 0 && $cli ne "";
      }
    }
  ' "$CHANGE_LOG" > "$INCIDENTS_TSV" 2>/dev/null || : > "$INCIDENTS_TSV"
  return 0
}

# FIX-RESCUE-10: monthly rotation of the prose change-log so it never grows
# unbounded. When the calendar month rolls over, the accumulated change-log.md is
# archived to change-log-<prev-month>.md and a fresh header starts the new month.
# The compact incidents.tsv (read by pattern_flag) is NOT rotated -- it stays
# small and is the durable index across months.
rotate_change_log_if_month_rolled() {
  local cur_month marker_month
  cur_month=$(date +%Y-%m)
  marker_month=""
  [ -f "$CHANGE_LOG_MONTH_MARKER" ] && marker_month=$(cat "$CHANGE_LOG_MONTH_MARKER" 2>/dev/null)
  if [ -z "$marker_month" ]; then
    printf '%s' "$cur_month" > "$CHANGE_LOG_MONTH_MARKER" 2>/dev/null || true
    return 0
  fi
  [ "$marker_month" = "$cur_month" ] && return 0
  # Month rolled: archive the old month's prose, start a fresh change-log.md.
  if [ -f "$CHANGE_LOG" ] && [ ! -e "${ROOT}/change-log-${marker_month}.md" ]; then
    mv "$CHANGE_LOG" "${ROOT}/change-log-${marker_month}.md" 2>/dev/null || true
  fi
  printf '# Fleet remediation change-log — %s\n' "$cur_month" > "$CHANGE_LOG" 2>/dev/null || true
  printf '%s' "$cur_month" > "$CHANGE_LOG_MONTH_MARKER" 2>/dev/null || true
  return 0
}

# Pattern detection from the compact incidents.tsv index (epoch \t client \t
# class \t outcome), reading ONLY the recent window in a single awk pass -- no
# more O(n) perl re-scans of the multi-MB prose log on every fix.
#   REPEAT       = same client + same class in last 7 days, >=2 occurrences (including this one)
#   FLEET-WIDE   = same class hit on >=3 distinct clients in last 30 days (including this one)
pattern_flag() {
  local class="$1"
  local now_epoch
  now_epoch=$(date +%s)
  ...
  backfill_incidents_tsv_if_missing
  [ -f "$INCIDENTS_TSV" ] || { echo "NONE"; return; }

  # Single awk pass over the recent window. selfmonth=1 iff THIS client already
  # logged THIS class within 30 days (mirrors the old distinct-set membership check).
  local counts r d selfm
  counts=$(awk -F'\t' -v cls="$class" -v cli="$CLIENT" -v wk="$week_ago" -v mo="$month_ago" '
    $1 ~ /^[0-9]+$/ && $3 == cls {
      if ($1 >= wk && $2 == cli) repeat++
      if ($1 >= mo) {
        if (!(seen[$2]++)) distinct++
        if ($2 == cli) selfmonth = 1
      }
    }
    END { printf "%d %d %d", repeat+0, distinct+0, selfmonth+0 }
  ' "$INCIDENTS_TSV" 2>/dev/null)
  r=$(printf '%s' "$counts" | awk '{print $1}'); [ -z "$r" ] && r=0
  d=$(printf '%s' "$counts" | awk '{print $2}'); [ -z "$d" ] && d=0
  selfm=$(printf '%s' "$counts" | awk '{print $3}'); [ -z "$selfm" ] && selfm=0

  # +1 for the entry we're about to append.
  local r_incl=$((r + 1))
  local d_incl=$d
  # If this client never logged this class in the last 30 days, the new entry
  # adds a distinct client to the fleet-wide count.
  [ "$selfm" -eq 0 ] && d_incl=$((d + 1))
...
append_change_log() {
  ...
  local ts
  ts=$(date "+%Y-%m-%d %H:%M %Z")
  # FIX-RESCUE-10: rotate the prose log monthly BEFORE appending, and write the
  # compact index line pattern_flag reads. The tsv line carries no secrets
  # (epoch, client id, failure class, outcome only).
  rotate_change_log_if_month_rolled
  printf '%s\t%s\t%s\t%s\n' "$(date +%s)" "$CLIENT" "$class" "$outcome" >> "$INCIDENTS_TSV" 2>/dev/null || true
...
# FIX-RESCUE-10 testability: REMEDIATE_LIB_ONLY=1 sources this file for its pure
# helpers (pattern_flag, rotate_change_log_if_month_rolled, append_change_log,
# backfill_incidents_tsv_if_missing) WITHOUT running the live remediation flow.
# Production never sets it.
if [ "${REMEDIATE_LIB_ONLY:-0}" = "1" ]; then
