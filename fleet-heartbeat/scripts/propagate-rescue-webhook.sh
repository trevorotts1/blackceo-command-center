...
MAC_ROSTER=(
  ...
  "jill-bulluck|rescue-jill-bulluck|Jill-agent"
...
# $1 = client label (used in the example payload), $2 = agent display name.
# No em dashes. Marker line is used for idempotency: "## Escalate to Rescue Rangers".
make_section() {
  local client="$1" agent="$2"
  cat <<SECTION
...
curl -s -X POST "\$RESCUE_RANGERS_WEBHOOK_URL" -H "Content-Type: application/json" \\
  ...
  -d '{"action":"escalate","client":"${client}","agent":"${agent}","message":"<concise problem + what you already tried + your EXACT OpenClaw version>"}'
\`\`\`

- \`RESCUE_RANGERS_WEBHOOK_URL\` is set in your environment (\`.env\` + \`openclaw.json\` env.vars). If it is missing, that itself is a setup gap; report it to Trevor's chat \`5252140759\`.
- \`RESCUE_RANGERS_WEBHOOK_SECRET\` is set in your environment alongside the URL. Include it as the \`X-Rescue-Secret\` header. The \${...:+} syntax skips the header gracefully if the var is empty (backward-compatible).
- Keep \`message\` concise: the problem, the fixes you already tried, and your exact OpenClaw version (\`openclaw --version\`).
- Never put real secrets (API keys, tokens, root passwords) in the message. Reference the env var name instead.

**The loop:** Rescue Rangers reads your escalation, works the fix, and posts the answer back into the Rescue Rangers group thread. Apply the fix it gives you. When the fix works, close the loop by POSTing the resolution signal:

\`\`\`
curl -s -X POST "\$RESCUE_RANGERS_WEBHOOK_URL" -H "Content-Type: application/json" \\
  \${RESCUE_RANGERS_WEBHOOK_SECRET:+-H "X-Rescue-Secret: \$RESCUE_RANGERS_WEBHOOK_SECRET"} \\
  -d '{"action":"escalate","client":"${client}","agent":"${agent}","message":"✅ RESOLVED: <one-line what fixed it>"}'
\`\`\`
...
# 3. AGENTS.md section (insert before AGENTS_ANCHOR, else append). Idempotent on
#    the "## Escalate to Rescue Rangers" marker.
if [ -n "${AGENTS_FILE}" ] && [ -f "${AGENTS_FILE}" ]; then
  if grep -q "## Escalate to Rescue Rangers" "${AGENTS_FILE}"; then
    echo "  agents: section already present; skipping insert"
  else
...
sec=open(secf).read()
txt=open(src).read()
if "## Escalate to Rescue Rangers" in txt:
    print("  agents: already present (race); skip")
else:
...
if [[ "${MAC_ONLY}" != "1" ]]; then
  for entry in "${VPS_ROSTER[@]}"; do
    ...
    section_b64=$(make_section "${client}" "${agent}" | base64 | tr -d '\n')
...
# Propagate RESCUE_RANGERS_WEBHOOK_SECRET (shared auth secret for the webhook entry).
if [ -n "\${SECRET_VAL}" ]; then
  ...
  docker exec "\${CONTAINER}" sh -c "
    if command -v jq >/dev/null 2>&1; then
      jq --arg k '\${SECRET_KEY}' --arg v '\${SECRET_VAL}' '.env = (.env // {}) | .env.vars = (.env.vars // {}) | .env.vars[\$k] = \$v' /data/.openclaw/openclaw.json > /data/.openclaw/openclaw.json.tmp && mv /data/.openclaw/openclaw.json.tmp /data/.openclaw/openclaw.json && echo '  container openclaw.json: secret patched'
