...
#   2. If 200 OK: clear both alarm flags + the restart counter (re-arms for the
#      next down-episode) and exit.
...
#      not help) and post ONE distinct crash-loop escalation telling the operator
#      to check rescue-receiver-run.sh / secrets .env.
#   Both alarms are flag-file deduped (fire once per episode) and re-arm only on
#   a 200 (down->up transition), so the operator topic is never spammed.
#   FIX-RESCUE-02: this whole alarm+crash-loop path was previously dead
#   (BOT_TOKEN hardcoded to "" and the receiver kickstarted forever in silence).
#
