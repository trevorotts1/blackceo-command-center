...
- **Inbox:** 171 tasks (was 183; 4 Marketing blockers unblocked + 1 client task re-queued)
- **In Progress:** 8
- **Review:** 2
- **Blocked:** 0
...
4. **Re-queued a client's box-check task** `fbcc4794` from blocked → inbox now that OpenClaw Maintenance runtime exists.
...
- **Presentations pipeline:** 44 inbox tasks, many E2E/routing tests. One in-progress deck rebuild for a client (`5dd2d4d7`).
- **Fleet checks:** Several in-progress status checks (some clients already done).
