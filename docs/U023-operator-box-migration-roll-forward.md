...
> **Path note:** All paths below use `~/command-center` as a placeholder for the blackceo-command-center repo checkout on the operator box. Replace `~/command-center` with the actual checkout path on the operator's machine (e.g. `~/projects/command-center` or wherever the repo lives).

## Pre-flight: check current migration count

```bash
sqlite3 ~/command-center/data/mission-control.db "SELECT COUNT(*) FROM _migrations;"
```
...
3. Manually list pending migrations: `sqlite3 ~/command-center/data/mission-control.db "SELECT * FROM _migrations ORDER BY id DESC LIMIT 5;"`
