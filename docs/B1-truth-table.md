...
| 35 | Disk free space reported ≥ 500 MB but the check path resolves to a separate large-capacity mount (`/data`); the actual CC filesystem partition has < 500 MB free | FAIL | disk: wrong-mount false-green — check path resolves to separate mount | — | **Sheila incident root cause**: /data is a large bind-mount; `process.cwd()` and `DATABASE_PATH` dir resolve to `/data` on a misconfigured box, reporting abundant free space on the wrong partition. deep-checks.ts explicitly prevents this (`checkPath` from `DATABASE_PATH` dir or `process.cwd()`, never hard-coded `/data`). **Vitest coverage (REDO #2)**: two tests in `tests/unit/deep-health.test.ts` prove the guard: (a) no DATABASE_PATH — check probes process.cwd() (300 MB, below threshold) not /data (80 GB) → FAIL; (b) DATABASE_PATH set to CC partition path — check probes DATABASE_PATH dir (300 MB) not /data → FAIL. Both assert `result.path` does not start with `/data`. |
