...
| 35b | `DATABASE_PATH` unset AND `process.cwd()` is under `/data` (e.g. `/data/app` — a Docker container where the CC app root IS the bind-mount) | FAIL | disk: wrong-mount false-green — CWD resolves to known large bind-mount | — | **REDO-REDO PRIMARY FALSE-GREEN FIX (Row 35 CWD variant)**: the REDO #1 fix only guarded the `DATABASE_PATH` branch inside `resolveCheckPath()`. When `DATABASE_PATH` is unset and `process.cwd()` is itself under `/data`, the old code fell through to `return process.cwd()` with no mount check. `diskReader.readFreeBytes('/data/app')` queried the Docker volume (~80 GB free) → `checkDiskHeadroom()` returned `pass:true`. The existing Row 35 vitest tests never exposed this because `vi.spyOn(process,'cwd')` always set CWD to a `/tmp/...` path — not `/data/...`. Fix: `resolveCheckPath()` now chooses the candidate first (`envPath ? dirname(envPath) : process.cwd()`), THEN evaluates `WRONG_MOUNT_PREFIXES` against the candidate regardless of which branch produced it. Returns `null` when wrong-mount; `checkDiskHeadroom()` returns an immediate FAIL on `null`. vitest: `DATABASE_PATH` unset + `process.cwd()` mocked to `/data/app` + diskReader returns 80 GB for `/data/...` → `pass=false`, detail matches wrong-mount guard message. |
