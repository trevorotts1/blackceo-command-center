...
| 32 | `NEXT_PUBLIC_APP_URL` set to a non-localhost URL AND `CC_PUBLIC_URL` unset (cannot verify) OR `CC_PUBLIC_URL` set to a different hostname | FAIL | config: NEXT_PUBLIC_APP_URL unverifiable or mismatch — SSE and webhooks will fail | — | **REDO #1 fix:** old code fell through to `pass=true` for any valid non-localhost URL when `CC_PUBLIC_URL` was unset. A wrong tunnel URL (e.g. copied from another client) would pass silently. Fix: non-localhost + `CC_PUBLIC_URL` unset → FAIL (cannot verify). Non-localhost + `CC_PUBLIC_URL` different host → FAIL (confirmed mismatch). |
