...
1. **Replicate provider KEPT (P1-14).** The v4.0.1 post-build review
   (`$HOME/Downloads/BLACKCEO-V4-POST-BUILD-FIXES.md`
...
| Client | Subdomain | CF Access App UUID | AUD | PM2 entry | Telegram (Operator / Client) |
|---|---|---|---|---|---|
| `<client-1>` | `<client-1>.zerohumanworkforce.com` | `<app-uuid-1>` | `<aud-1>` | command-center (dupe mission-control deleted) | `<chat-id>` / `<chat-id>` |
| `<client-2>` | `<client-2>.zerohumanworkforce.com` | `<app-uuid-2>` | `<aud-2>` | command-center (dupe mission-control deleted) | `<chat-id>` / `<chat-id>` |
| `<client-3>` | `<client-3>.zerohumanworkforce.com` | `<app-uuid-3>` | `<aud-3>` | mission-control (canonical, no rename) | `<chat-id>` / `<chat-id>` |

(The real client labels, subdomains, Access App UUIDs, AUD tags, and Telegram chat IDs are operator-local data and are intentionally not committed to this fleet-wide repo; look them up in the operator's accounts ledger.)

OTP IdP: `<otp-idp-uuid>` (already existed on the account).
Team domain: `<cf-team-domain>.cloudflareaccess.com`.
...
- [ ] `<client-1>` — replace `trevor@blackceo.com` with the client's real email on app `<app-uuid-1>`.
- [ ] `<client-2>` — replace `trevor@blackceo.com` with the client's real email on app `<app-uuid-2>`.
- [ ] `<client-3>` — replace `trevor@blackceo.com` with the client's real email on app `<app-uuid-3>`.

### Phase 3 lesson — PM2 dupe trap on two of the client boxes

Two of the three client boxes had a `command-center` PM2 entry (the actually-running app on port 4000) AND a dormant/stopped `mission-control` entry left over from a prior boot. Running `pm2 reload ecosystem.config.cjs` (whose `name: 'mission-control'`) would relaunch the dupe and create a port-4000 conflict. Fix used: `pm2 delete mission-control` then `pm2 reload command-center --update-env`. The third box's container already had only `mission-control` (matching ecosystem.config.cjs), so the standard `pm2 reload ecosystem.config.cjs` was safe there.
