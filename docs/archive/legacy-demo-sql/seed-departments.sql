-- Mission Control Task Redistribution Script
-- 1. Move existing tasks to correct department workspaces
-- 2. Add new sample tasks for variety
-- 3. Create matching events for Live Feed

-- ============================================
-- PART 1: REDISTRIBUTE EXISTING TASKS
-- ============================================

-- Move tasks to their correct workspace based on department field
UPDATE tasks SET workspace_id = department WHERE department IS NOT NULL;

-- ============================================
-- PART 2: ADD NEW SAMPLE TASKS (with UUIDs)
-- ============================================
...
('mkt-task-02', 'Update brand guidelines', 'Refresh logo usage and color palette docs', 'backlog', 'normal', 'marketing', 'marketing', datetime('now'), datetime('now')),
('mkt-task-03', 'SEO audit', 'Technical SEO review of main site', 'testing', 'medium', 'marketing', 'marketing', datetime('now'), datetime('now')),
...
('sales-task-02', 'Update CRM', 'Clean up duplicate contacts', 'backlog', 'low', 'sales', 'sales', datetime('now'), datetime('now')),
('sales-task-03', 'Sales deck refresh', 'Update pricing slides', 'assigned', 'medium', 'sales', 'sales', datetime('now'), datetime('now')),
('sales-task-04', 'Demo preparation', 'Prepare custom demo for enterprise client', 'in_progress', 'critical', 'sales', 'sales', datetime('now'), datetime('now'));
...
('sup-task-02', 'Update knowledge base', 'Add FAQ for new feature', 'backlog', 'normal', 'support', 'support', datetime('now'), datetime('now')),
('sup-task-03', 'VIP escalation', 'Handle enterprise customer issue', 'assigned', 'critical', 'support', 'support', datetime('now'), datetime('now'));

-- Operations Tasks
INSERT INTO tasks (id, title, description, status, priority, workspace_id, department, created_at, updated_at) VALUES
('ops-task-01', 'Vendor contract review', 'Renegotiate hosting agreement', 'in_progress', 'medium', 'operations', 'operations', datetime('now'), datetime('now')),
('ops-task-02', 'Office supply inventory', 'Restock kitchen and supplies', 'backlog', 'low', 'operations', 'operations', datetime('now'), datetime('now')),
...
('cre-task-02', 'Brand refresh concepts', 'Explore new visual direction', 'backlog', 'normal', 'creative', 'creative', datetime('now'), datetime('now')),
('cre-task-03', 'Social assets', 'Create Instagram carousel templates', 'testing', 'medium', 'creative', 'creative', datetime('now'), datetime('now')),
...
('hr-task-03', 'Performance reviews', 'Schedule Q1 review meetings', 'assigned', 'medium', 'hr', 'hr', datetime('now'), datetime('now'));

-- Legal Tasks
INSERT INTO tasks (id, title, description, status, priority, workspace_id, department, created_at, updated_at) VALUES
('legal-task-01', 'Contract review', 'Review partnership agreement', 'in_progress', 'high', 'legal', 'legal', datetime('now'), datetime('now')),
('legal-task-02', 'Privacy policy update', 'Update for new regulations', 'backlog', 'medium', 'legal', 'legal', datetime('now'), datetime('now')),
('legal-task-03', 'Trademark filing', 'Submit application for new product name', 'assigned', 'critical', 'legal', 'legal', datetime('now'), datetime('now'));
...
('web-task-02', 'Performance optimization', 'Improve Core Web Vitals scores', 'testing', 'medium', 'webdev', 'webdev', datetime('now'), datetime('now')),
('web-task-03', 'Bug fixes', 'Fix mobile navigation issues', 'backlog', 'normal', 'webdev', 'webdev', datetime('now'), datetime('now')),
...
('app-task-03', 'Crash analytics', 'Integrate Sentry error tracking', 'backlog', 'normal', 'appdev', 'appdev', datetime('now'), datetime('now'));

-- Graphics Tasks
INSERT INTO tasks (id, title, description, status, priority, workspace_id, department, created_at, updated_at) VALUES
('gfx-task-01', 'Logo variations', 'Create monochrome and reversed versions', 'done', 'medium', 'graphics', 'graphics', datetime('now'), datetime('now')),
...
('vid-task-02', 'Testimonial edits', 'Cut customer interview footage', 'backlog', 'normal', 'video', 'video', datetime('now'), datetime('now')),
('vid-task-03', 'Social clips', 'Create 15-second promo shorts', 'testing', 'medium', 'video', 'video', datetime('now'), datetime('now')),
...
('aud-task-02', 'Background music', 'Compose for product video', 'backlog', 'normal', 'audio', 'audio', datetime('now'), datetime('now')),
('aud-task-03', 'Voice over', 'Record VO for tutorial', 'assigned', 'medium', 'audio', 'audio', datetime('now'), datetime('now'));

-- Research Tasks
INSERT INTO tasks (id, title, description, status, priority, workspace_id, department, created_at, updated_at) VALUES
('res-task-01', 'Market analysis', 'Competitor feature comparison', 'in_progress', 'high', 'research', 'research', datetime('now'), datetime('now')),
('res-task-02', 'User interviews', 'Schedule and conduct 10 sessions', 'assigned', 'medium', 'research', 'research', datetime('now'), datetime('now')),
('res-task-03', 'Trend report', 'Compile emerging tech trends Q2', 'backlog', 'normal', 'research', 'research', datetime('now'), datetime('now'));
...
('com-task-03', 'Crisis protocol', 'Review and update response plan', 'assigned', 'critical', 'comms', 'comms', datetime('now'), datetime('now'));

-- ============================================
-- PART 3: CREATE EVENTS FOR LIVE FEED
-- ============================================

INSERT INTO events (id, type, task_id, message, created_at) VALUES
('evt-001', 'task_created', 'ceo-task-01', 'New task created: Define quarterly vision', datetime('now')),
...
('evt-006', 'task_created', 'sales-task-04', 'New critical task: Demo preparation', datetime('now')),
...
('evt-010', 'task_created', 'sup-task-03', 'New critical task: VIP escalation', datetime('now')),
...
('evt-016', 'task_created', 'legal-task-03', 'New critical task: Trademark filing', datetime('now')),
...
('evt-032', 'task_created', 'com-task-03', 'New critical task: Crisis protocol', datetime('now'));
